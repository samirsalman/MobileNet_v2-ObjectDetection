...
class DetectionModel:
    def __init__(self, pbtext_path: str, weights_path: str, classnames_path: str, width: int = 320, height: int = 320,
                 scale_ratio: float = 127.5, mean: float = 127.5, yolo:bool=False):
        self.yolo=yolo
    ...
    def detect(self, img: np.ndarray) -> np.ndarray:
        ...
        if len(classIds) != 0:
            for classId, confidence, box in zip(classIds.flatten(), confidences.flatten(), bbox):
                ...
                if self.yolo:
                    class_prev=0
                cv2.putText(img, self.classNames[classId - class_prev], (box[0] + 10, box[1] + 30), cv2.FONT_HERSHEY_COMPLEX, 1,
                            (0, 255, 0),
                            2)
